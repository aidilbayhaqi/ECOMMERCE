...
const Phone = () => {
  ...
  const getProducts = async () => {
    ...
    setProducts(
      response.data
        .filter((product) => product.categories === "PHONE")
        .slice(0, 6)
    );
  };
  ...
  return (
    <div className="relative w-full p-10 mt-5 items-center justify-center text-center gap-5 ">
      <div className="justify-between px-5 flex flex-col md:flex-row mb-5">
        <h1 className="font-bold text-4xl">PHONE</h1>
        <Link href={`/categories/phone`}>See All</Link>
      </div>
